...
#
#
def Extract_EMGs_CSV(object_id='all',object_type='all', transition='all', flag_include='yes',verbose='no',u='yes'):
	...
	data = np.zeros((1000,),dtype=[('ID',list), ('type',list), ('transition',list), ('z',float), ('ez',float), \
								   ...
								   ('L_transition',float), ('eL_transition',float), ('magnification',list), \
	...
	if flag_include == 'yes':
		...
		data=data[indices_keep]
	if u == 'yes':
		indices_keep=[i for i,x in enumerate(data['magnification']) if len(x) == 1 and '1' in x]
		data=data[indices_keep]
	if u == 'no':
		indices_keep=[i for i,x in enumerate(data['magnification']) if len(x) != 1 or '5' in x  or '9' in x]
		data=data[indices_keep]
	if verbose == 'yes':
		List_EMGs(data)
